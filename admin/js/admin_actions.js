$(document).ready(function () {
  ...
  $("#adminLogin").submit(function (e) {
    ...
    $(".alert").remove(); // Clear any existing alerts
      ...
      success: function (response) {
        ...
        if (res.hasOwnProperty("sub_category")) {
          ...
          for (var i = 0; i < sub_cat_length; i++) {
            sub_cat +=
              '<option value="' +
              res.sub_category[i].sub_cat_id +
              '">' +
              res.sub_category[i].sub_cat_title +
              "</option>";
          }
        ...
        if (res.hasOwnProperty("brands")) {
          ...
          if (brand_length > 0) {
            for (var j = 0; j < brand_length; j++) {
              brand +=
                '<option value="' +
                res.brands[j].brand_id +
                '">' +
                res.brands[j].brand_title +
                "</option>";
            }
          } else {
            brand =
              '<option value="" selected disabled>No Brands Found</option>';
          }
  ...
  $("#createProduct").submit(function (e) {
    ...
    if (title == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Title Field is Empty.</div>'
      );
    } else if (cat == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Category Field is Empty.</div>'
      );
    } else if (sub_cat == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Sub Category Field is Empty.</div>'
      );
    } else if (des == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Description Field is Empty.</div>'
      );
    } else if (price == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Price Field is Empty.</div>'
      );
    } else if (qty == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Quantity Field is Empty.</div>'
      );
    } else if (image == "") {
      $("#createProduct").prepend(
        '<div class="alert alert-danger">Image Field is Empty.</div>'
      );
    } else {
      var formdata = new FormData(this);
      formdata.append("create", 1);
      console.log(formdata.append("create", 1));
      $.ajax({
        url: "./php_files/products.php",
        type: "POST",
        data: formdata,
    ...
    if (cat == "") {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (res.hasOwnProperty("success")) {
            $("#updateCategory").prepend(
              '<div class="alert alert-success">Category Modified Successfully.</div>'
            );
            setTimeout(function () {
              window.location = URL + "admin/category.php";
            }, 1000);
  ...
  $("#createSubCategory").submit(function (e) {
    ...
    var parent = $(".cat_parent option:selected").val();
    if (title == "") {
      $("#createSubCategory").prepend(
        '<div class="alert alert-danger">Title Field is Empty.</div>'
      );
    } else if (parent == "") {
      $("#createSubCategory").prepend(
        '<div class="alert alert-danger">Parent Category Field is Empty.</div>'
      );
    } else {
      var formdata = new FormData(this);
      formdata.append("create", "1");
    ...
    if (title == "") {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (res.hasOwnProperty("success")) {
            $("#updateSubCategory").prepend(
              '<div class="alert alert-success">Sub Category Modified Successfully.</div>'
            );
            setTimeout(function () {
              window.location = URL + "admin/sub_category.php";
            }, 1000);
    ...
    if (title == "") {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (res.hasOwnProperty("success")) {
            $("#updateBrand").prepend(
              '<div class="alert alert-success">Brand Modified Successfully.</div>'
            );
            setTimeout(function () {
              window.location = URL + "admin/brands.php";
            }, 1000);
      ...
      success: function (response) {
        ...
        if (response.length > 0) {
          ...
          var tr =
            ...
            "<td>User Status</td>" +
            "<td>" +
            (user.user_role == "1" ? "Activated" : "Blocked") +
            "</td>" +
            "</tr>" +
            "</table>";
          $("#user-detail .modal-body").html(tr); // Clear and append new content
  ...
  $("#updateOptions").submit(function (e) {
    ...
    if (site_name == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Site Name Field is Empty.</div>'
      );
    }
    if (site_title == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Site Title Field is Empty.</div>'
      );
    } else if (footer_text == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Footer Text Field is Empty.</div>'
      );
    } else if (currency == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Currency Format Field is Empty.</div>'
      );
    } else if (desc == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Site Description is empty Field is Empty.</div>'
      );
    } else if (phone == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Phone Field is Empty.</div>'
      );
    } else if (email == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Email Field is Empty.</div>'
      );
    } else if (address == "") {
      $("#updateOptions").prepend(
        '<div class="alert alert-danger">Address Field is Empty.</div>'
      );
    } else {
      var formdata = new FormData(this);
      formdata.append("update", 1);
      $.ajax({
        ...
        success: function (response) {
          ...
          if (res.hasOwnProperty("success")) {
            $("#updateOptions").prepend(
              '<div class="alert alert-success">Options Updates Successfully.</div>'
            );
            setTimeout(function () {
              window.location();
            }, 1000);
