...
if(($_GET['payment_request_id'] == $_SESSION['TID']) && $_GET['payment_status'] == 'Credit'){
	...
	$response = '<div class="panel-body">
	  ...
	  // reduce purchased quantity from products
        $order_products ="SELECT product_id,product_qty, pay_req_id FROM order_products WHERE pay_req_id ='{$_GET['payment_request_id']}'";
	   $result = mysqli_query($conn,$order_products)or die("Query Failed");
