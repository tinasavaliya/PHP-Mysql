...
$sql1 = "SELECT sub_cat_id, sub_cat_title FROM sub_categories WHERE sub_cat_id = '{$cat}'";
$result1 = mysqli_query($conn, $sql1) or die("Query Failed");

// Check if there are rows returned
if (mysqli_num_rows($result1) > 0) {
    $rows1 = mysqli_fetch_all($result1, MYSQLI_ASSOC);
    $row1 = $rows1[0]; // Assuming you only need the first row

    // Update $title and $page_head
    $title = $row1['sub_cat_title'] . ' : Buy ' . $row1['sub_cat_title'] . ' at Best Price<br>';
    $page_head = $row1['sub_cat_title'];
} else {
    $title = "Result Not Found";
    $page_head = "Result Not Found";
}
?>
        ...
        <?php if (!empty($rows1)) { ?>
            <div class="row">
                <div class="col-lg-2 col-md-6 col-12">
                    <div class="left-sidebar mb-md-0 mb-3">
                        <h3>Related Brands</h3>
                        <?php
                        $sql2 = "SELECT cat_parent FROM sub_categories WHERE sub_cat_id = '{$cat}'";
                        $result2 = mysqli_query($conn, $sql2) or die("Query Failed");
                        $row2 = mysqli_fetch_assoc($result2);

                        $sql3 = "SELECT * FROM brands WHERE brand_cat = '{$row2["cat_parent"]}'";
                        $result3 = mysqli_query($conn, $sql3) or die("Query Failed");
                        $rows3 = mysqli_fetch_all($result3, MYSQLI_ASSOC);

                        if (!empty($rows3)) { ?>
                            <ul>
                                <?php foreach ($rows3 as $res2) { ?>
                                    <li><a href="brands.php?brand=<?php echo $res2['brand_id']; ?>"><?php echo $res2['brand_title']; ?></a></li>
                                <?php } ?>
                            </ul>
                        <?php } ?>
                    </div>
                ...
                <div class="col-md-9 col-12">
                    <?php
                    $limit = 8;
                    $sql4 = "SELECT * FROM products WHERE product_sub_cat = '{$cat}' AND product_status = 1 AND qty > 0 LIMIT $limit";
                    $result4 = mysqli_query($conn, $sql4) or die("Query Failed");
                    $rows4 = mysqli_fetch_all($result4, MYSQLI_ASSOC);

                    if (!empty($rows4)) { ?>
                        <div class="row"><?php
                            foreach ($rows4 as $res4) { ?>
                    ...
                    <div class="col-md-12 pagination-outer">
                        <?php
                        $total_pages_sql = "SELECT COUNT(*) FROM products WHERE product_sub_cat = '{$cat}' AND product_status = 1 AND qty > 0";
                        ...
                        for ($i = 1; $i <= $total_pages; $i++) {
                            if ($total_rows == $limit) {
                                ...
                                echo '<li><a href="?cat=' . $cat . '&page=' . $i . '">' . $i . '</a></li>';
                            }
                        }
                        echo '</ul>';
                        ?>
